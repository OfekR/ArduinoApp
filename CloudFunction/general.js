...
exports.handleGate = functions.https.onRequest((req, res) => {
    ...
    .catch(error => {
      ...
      */
      ...
      exports.playerHit = functions.https.onRequest((req, res) => {
        let playerId = Number(req.query.id);
        var playerString = (playerId === 1) ? "Player1" : "Player2"
        var fireBasePath = "/RfidReading/" + playerString;

        admin.database().ref(fireBasePath).update({gotHit: 1});
        return res.status(200).send("hitOk")
        });